
import React, { Component } from "react";
import { StyleSheet, View, Text, Image, StatusBar, Dimensions, TouchableOpacity, FlatList, SafeAreaView } from "react-native";
import config from "./../config/conf";
const win = Dimensions.get('window');

export default class Shop extends Component {
    defaultImage = "data:image/png;base64,";
    state = {
        id: false,
        errorText: "",
        defaultData: "",
        image: this.defaultImage,
        content: []
    }

    async componentDidMount() {
        let shop = this.props.navigation.getParam('shopID', false);
        if (shop) {
            fetch(`${config.host}/api/getShops/${shop}/1`, {
                method: 'GET'
            }).then((response) => { return response.json() }).then(async (data) => {
                if (data.response.status == "ok") {
                    this.setState({
                        id: shop,
                        errorText: "",
                        defaultData: data.response.data[0],
                        image: `${config.host}/${data.response.data[0]['image']}`
                    });
                    this.loadProduct()
                } else {
                    this.setState({ errorText: data.response.info })
                }
            }).catch((e) => {
                this.setState({ errorText: "Ошибка соединения" })
            })
        } else {
            this.setState({ errorText: "id undefined" })
        }
    }

    async loadProduct() {
        fetch(`${config.host}/api/getProduct/${this.state.content.length || 1}/20?word=${this.state.id}`, {
            method: 'GET'
        }).then((response) => { return response.json() }).then(async (data) => {
            if (data.response.status == "ok") {
                this.setState({ content: [...this.state.content, ...data.response.data] })
            } else {
                this.setState({ errorText: data.response.info })
            }
        }).catch((e) => {
            this.setState({ errorText: "Ошибка соединения" })
        })
    }

    render(props) {
        return (
            <View style={styles.container}>
                <View style={{ flex: 1, zIndex: 50 }}>
                    <StatusBar
                        barStyle="dark-content"
                        backgroundColor="rgba(242,83,105,1)"
                    />
                    <View onScroll={this.renderProduct}>
                        <View style={styles.topBlock}>
                            <Image
                                resizeMode={'contain'}
                                style={{
                                    flex: 1,
                                    alignSelf: 'stretch',
                                    width: win.width,
                                    height: win.height,
                                }}
                                source={{
                                    uri: this.state.image
                                }}
                            />
                        </View>
                        <View style={styles.blockLabel}>
                            <Text style={styles.text}>
                                {this.state.errorTex ? this.state.errorTex : (this.state.defaultData['name'] ? this.state.defaultData['name'] : "")}
                            </Text>
                        </View>
                        <View style={[styles.blockLabel, { backgroundColor: "#00ffb8" }]}>
                            <Text style={[styles.text, { fontSize: 20 }]}>
                                {this.state.defaultData['info'] ? this.state.defaultData['info'] : ""}
                            </Text>
                        </View>
                        <FlatList
                            style={{ marginTop: 20 }}
                            data={this.state.content}
                            keyExtractor={(item) => item.name}
                            numColumns={2}
                            contentContainerStyle={{ minHeight: `100%` }}
                            scrollEnabled={true}
                            renderItem={
                                ({ item }) => {
                                    return (
                                        <TouchableOpacity key={item.name} onPress={() => { this.props.navigation.navigate("Product", { ProductID: item.id }) }} style={styles.blockProduct}>
                                            <Image
                                                resizeMode={'contain'}
                                                style={{ flex: 1, alignSelf: 'stretch', width: 160, height: win.height }}
                                                source={{
                                                    uri: item.image ? `${config.host}/${item.image}` : this.defaultImage
                                                }}
                                            />
                                            <View style={{ padding: 5, display: "flex", flexDirection: "row", justifyContent: "space-between" }}>
                                                <View>
                                                    <Text style={{ fontSize: 16, color: "gray" }}>{item.name}</Text>
                                                </View>
                                                <View>
                                                    <Text style={{ fontSize: 16, color: "#ff4f00" }}>{`${item.price} ₽`}</Text>
                                                </View>
                                            </View>
                                        </TouchableOpacity>
                                    )
                                }
                            }
                        />
                    </View>
                </View>
            </View>
        );
    }
}

const styles = StyleSheet.create({
    container: {
        flex: 1,
        backgroundColor: "rgba(51,51,51,1)",
    },
    topBlock: {
        backgroundColor: "rgba(242,83,105,1)",
        width: 360,
        height: 200,
        borderBottomLeftRadius: 25,
        borderBottomRightRadius: 25,
        padding: 10
    },
    cupertinoFooter2: {
        width: 360,
        height: 51,
        backgroundColor: "#f9f9f9",
    },
    text: {
        color: "#efefef",
        fontSize: 26,
        fontWeight: "600",
        textAlign: "center"
    },
    blockLabel: {
        backgroundColor: "orange",
        width: 340,
        borderRadius: 25,
        marginTop: 15,
        paddingVertical: 10,
        marginLeft: 10
    },
    blockProduct: {
        width: 160,
        height: 160,
        backgroundColor: "#f5f5dc",
        borderRadius: 10,
        marginBottom: 20,
        marginLeft: 13
    }
});
